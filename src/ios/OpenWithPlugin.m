...
- (void) checkForFileToShare {
  ...
  // If filePath is set, load the file from the container
  if (filePath && [filePath length] > 0) {
    [self debug:[NSString stringWithFormat:@"Loading file from container: %@", filePath]];
    
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSURL *containerURL = [fileManager containerURLForSecurityApplicationGroupIdentifier:SHAREEXT_GROUP_IDENTIFIER];
    
    if (containerURL) {
      NSURL *fileURL = [containerURL URLByAppendingPathComponent:filePath];
      NSError *readError = nil;
      data = [NSData dataWithContentsOfURL:fileURL options:NSDataReadingMappedIfSafe error:&readError];
      
      if (readError) {
        [self error:[NSString stringWithFormat:@"Error reading file: %@", readError.localizedDescription]];
        return;
      }
      
      [self debug:[NSString stringWithFormat:@"Loaded %lu bytes from file", (unsigned long)data.length]];
  ...
  if ([type isEqual:@"text/vcard"]) {
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:@{
      ...
      @"exit": @YES,
      @"items": @[@{
        @"base64": [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding],
        @"type": type,
        @"utis": utis,
        @"uri": uri,
        @"name": name
...
// Cleanup old shared files
- (void) cleanupOldSharedFiles {
  [self debug:@"[cleanupOldSharedFiles]"];
  
  NSFileManager *fileManager = [NSFileManager defaultManager];
  NSURL *containerURL = [fileManager containerURLForSecurityApplicationGroupIdentifier:SHAREEXT_GROUP_IDENTIFIER];
  
  if (!containerURL) {
    [self error:@"Cannot access container for cleanup"];
    return;
  }
  
  NSError *error = nil;
  NSArray *files = [fileManager contentsOfDirectoryAtURL:containerURL
                              includingPropertiesForKeys:@[NSURLContentModificationDateKey]
                                                 options:NSDirectoryEnumerationSkipsHiddenFiles
                                                   error:&error];
  
  if (error) {
    [self error:[NSString stringWithFormat:@"Error reading container: %@", error.localizedDescription]];
    return;
  }
  ...
  for (NSURL *fileURL in files) {
    // Check only shared_* files
    if (![[fileURL lastPathComponent] hasPrefix:@"shared_"]) {
      continue;
    }
    ...
    NSTimeInterval age = [now timeIntervalSinceDate:modificationDate];
    
    if (age > maxAge) {
      [self debug:[NSString stringWithFormat:@"Deleting old file: %@", fileURL.lastPathComponent]];
      NSError *deleteError = nil;
      [fileManager removeItemAtURL:fileURL error:&deleteError];
      
      if (deleteError) {
        [self warn:[NSString stringWithFormat:@"Could not delete file: %@", deleteError.localizedDescription]];
      }
    }
  }
}
